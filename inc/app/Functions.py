...
class Functions():
    ...
    def getPathToFile(cls, path, fileStr, results0, results1, ext):
        result = path+"/" + \
        fileStr[(results0[3]+results0[4]):results1[1]]+"."+ext
    ...
    def seekStr(cls, fileStr):
        ...
        results0 = SeekString.seekArrayOfStrs(part0,True)                
        results1 = SeekString.seekArrayOfStrs(part1,False)
        resultEnline = SeekString.seekArrayOfStrsLessPos(JsonParams.getParamsEli())
        result=0
       
        if((results0[0] == True) and (results1[0] == True) and (resultEnline[0]==True and (results1[1] < resultEnline[1]))):                       
                stringResult=cls.getPathToFile(JsonParams.getPath(), fileStr, results0, results1, JsonParams.getParamExt())
                result = {"id": cls.countFounds, "strFound": JsonParams.getValuefromKey(), "posFound": SeekString.getLastPos(), "strFile": stringResult}            
        ...
        if (result==0): 
            Debug.print("(seekStr), Results = empty, (Not enough paramas)")
        return result
    ...
    def getAllStringInFileFromJsonDef(cls, fileStr):
        ...
        return Functions.getResultArrayPath()
